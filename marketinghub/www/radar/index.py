"""Dashboard /radar — sirve el HTML del zip LITERAL sin procesamiento Jinja.

El archivo real del diseño está en /assets/marketinghub/dashboards/radar/
(carpeta `public/dashboards/radar/`), servido como estático por Frappe.
Este handler solo redirige."""
import os
import frappe
...
def get_context(context):
	...
	# Servir el HTML del zip LITERAL desde /assets/ (archivo estático, sin procesamiento)
	frappe.local.flags.redirect_location = "/assets/marketinghub/dashboards/radar/index.html"
	raise frappe.Redirect


# ============ ENDPOINTS DEL DASHBOARD (mantener para JS externo) ============
...
def obtener_publicaciones_por_dia_semana(dias=90):
	...
	for i in range(7):
		c = counts.get(i, 0)
		out.append({"dow": i, "nombre": NOMBRES[i], "count": c,
...
def obtener_contadores():
	...
	try: guiones = frappe.db.count("Guion")
	except Exception: guiones = 0
	try: ads = frappe.db.count("Anuncio Competencia", filters={"esta_activo": 1})
	except Exception: ads = 0
	return {
